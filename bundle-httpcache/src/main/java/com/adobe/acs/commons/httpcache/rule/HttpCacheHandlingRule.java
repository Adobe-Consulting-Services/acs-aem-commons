 ...
 * <p>
 * Each method represents a hook to the http cache event.In case if a method is not applicable for a custom rule,
 * make it return true indicating the cache engine to continue processing the next rule.
 * </p>
 */
public interface HttpCacheHandlingRule {
    ...
    /**
     * Hook to supply custom behavior on {@link com.adobe.acs.commons.httpcache.engine.HttpCacheEngine} about to cache
     * a
     * response.
     *
     * @param request
     ...
     * handling rules being stopped and fallback action will be taken.
     */
    boolean onResponseCache(SlingHttpServletRequest request, SlingHttpServletResponse response);

    /**
     * Hook to supply custom behavior on {@link com.adobe.acs.commons.httpcache.engine.HttpCacheEngine} delivering
     * cache
     ...
     * handling rules being stopped and fallback action will be taken.
     */
    boolean onCacheDeliver(SlingHttpServletRequest request, SlingHttpServletResponse response);
