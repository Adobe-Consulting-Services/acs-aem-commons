...
@Component(service = WorkflowExternalProcess.class, property = {
...
public class TranscriptionProcess
      implements WorkflowExternalProcess, AudioHelper.AudioProcessor<MetaDataMap, Serializable> {
   ...
   @ObjectClassDefinition(name = "ACS AEM Commons - Watson Transcription Workflow Process", description = "ACS AEM Commons - Watson Transcription Workflow Process")
   public @interface Config {

      @AttributeDefinition(name = "Trancode Profile", description = "Profile name for audio transcoding. Must be a format acceptable to Watson", defaultValue = DEFAULT_PROFILE)
      String profile();
   }
   ...
   @Activate
   protected void activate(TranscriptionProcess.Config config) {
      this.profileName = StringUtils.defaultIfEmpty(config.profile(), DEFAULT_PROFILE);
   }
      ...
      if (profile != null) {
         ...
         try {
            ...
            try {
               ...
               log.error("Transcoded audio file @ " + transcodedAudio.getAbsolutePath() + " coud not be deleted",
                     e);
            }
         } catch (IOException e) {
   ...
   @Override
   public boolean hasFinished(Serializable serializable, WorkItem workItem, WorkflowSession workflowSession,
      ...
      if (serializable instanceof String) {
         ...
         if (result.isCompleted()) {
            try {
               asset.addRendition("transcription.txt",
                     new ByteArrayInputStream(result.getContent().getBytes("UTF-8")), "text/plain");
   ...
   @Override
   public void handleResult(Serializable serializable, WorkItem workItem, WorkflowSession workflowSession,
   ...
   private Asset getAssetFromPayload(WorkItem item, ResourceResolver resourceResolver) {
      if (item.getWorkflowData().getPayloadType().equals("JCR_PATH")) {
         ...
         if (resource != null) {
            ...
            log.error("Resource [{}] in payload of workflow [{}] does not exist.", path,
