...
public class RedirectMapModel {
	...
	private List<MapEntry> addItems(RedirectConfigModel config, Iterator<Resource> items, String suffix) {
		...
		while (items.hasNext()) {
			...
			Resource content = item.getChild(JcrConstants.JCR_CONTENT);
			if (content != null) {
	...
	private MapEntry toEntry(int index, String l) {
		...
		if (StringUtils.isBlank(l) || l.startsWith("#")) {
			...
			entry = new MapEntry(index, seg[0], seg[1], "File");
		} else if (seg.length > 2) {
			entry = new MapEntry(index, seg[0], seg[1], "File");
			entry.setValid(false);
			entry.setStatus(String.format(WHITESPACE_MSG, l));
		} else {
			entry = new MapEntry(index, seg[0], "", "File");
			entry.setValid(false);
