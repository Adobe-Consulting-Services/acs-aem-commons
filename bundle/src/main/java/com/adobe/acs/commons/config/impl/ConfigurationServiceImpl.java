...
@Component(label = "ACS AEM Commons - config service", description = "gets configuration object of the path requested", immediate = false, metatype = true)
@Service
public class ConfigurationServiceImpl implements ConfigurationsService {

    @Override
    public Configuration getConfiguration(Page page, String configPagePath) {
    ...
    public Configuration getConfiguration(Page page) {
        ...
        HierarchyNodeInheritanceValueMap hvm = new HierarchyNodeInheritanceValueMap(
                contentResource);
