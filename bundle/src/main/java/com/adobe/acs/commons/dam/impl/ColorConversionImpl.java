...
@Component(service=ColorConversion.class)
...
public final class ColorConversionImpl implements ColorConversion {
   ...
   @ObjectClassDefinition(name = "ACS AEM Commons - Color Conversion", description = "ACS AEM Commons - Color Conversion")
   public @interface Config {
      @AttributeDefinition(name = "CMYK ICC Profile", description = "ICC Profile for CMYK to RGB Conversion",
            defaultValue= {DEFAULT_CMYK_PROFILE},
            options = {
      ...
      String cmyk_icc_profile();
   }

    public static final String DEFAULT_CMYK_PROFILE = "JapanColor2001Coated";
    ...
    @Activate
    protected void activate(ColorConversionImpl.Config config) throws Exception {
        String profileName = config.cmyk_icc_profile();
        if (StringUtils.isEmpty(profileName)) {
              profileName = DEFAULT_CMYK_PROFILE;
        }
