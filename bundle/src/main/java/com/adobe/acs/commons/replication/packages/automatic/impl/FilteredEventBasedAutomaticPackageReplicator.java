...
@Designate(ocd = FilteredEventBasedAutomaticPackageReplicatorConfig.class, factory = true)
public class FilteredEventBasedAutomaticPackageReplicator implements EventHandler {

  @ObjectClassDefinition(name = "ACS AEM Commons - Automatic Package Replicator - Filtered Event-Based")
  public @interface FilteredEventBasedAutomaticPackageReplicatorConfig {
    ...
    @SuppressWarnings({ "squid:S00100" })
    @AttributeDefinition(name = "Event Filter", required = true)
    String event_filter();

    @AttributeDefinition(name = "Package Path", required = true)
    String packagePath();

    String webconsole_configurationFactory_nameHint() default "Filtered Event-Based Automatic Package Replicator <b>{event.topics}->{packagePath}</b>";
  }

  private static final Logger log = LoggerFactory.getLogger(FilteredEventBasedAutomaticPackageReplicator.class);

  private FilteredEventBasedAutomaticPackageReplicatorConfig config;

  @Reference
  private AutomaticPackageReplicator automaticPackageReplicator;

  @Activate
  public void activate(FilteredEventBasedAutomaticPackageReplicatorConfig cfg) {
    this.config = cfg;
  }
