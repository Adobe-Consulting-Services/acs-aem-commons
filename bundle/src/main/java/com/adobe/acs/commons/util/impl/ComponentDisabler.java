...
@Component(service = EventHandler.class, immediate = true, configurationPolicy = ConfigurationPolicy.REQUIRE, property = {
      "event.topics=[org/osgi/framework/BundleEvent/STARTED,org/osgi/framework/ServiceEvent/REGISTERED]" })
...
public class ComponentDisabler implements EventHandler {
   ...
   public @interface Config {
      @AttributeDefinition(name = "Disabled components", description = "The names of the components/services you want to disable", cardinality = Integer.MAX_VALUE)
      String[] components();
   }
   ...
   @Activate
   protected void activate(ComponentDisabler.Config config) {
      disabledComponents = config.components();
      handleEvent(null);
   }
