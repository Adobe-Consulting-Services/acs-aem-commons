...
public class TestServlets {
	...
	public void init() throws IOException, NoSuchFieldException {
		...
		doReturn("2").when(mockSlingRequest).getParameter("idx");
		doReturn("/source").when(mockSlingRequest).getParameter("source");
		doReturn("/target").when(mockSlingRequest).getParameter("target");
		doReturn("2").when(mockSlingRequest).getParameter("edit-id");
		doReturn("/source").when(mockSlingRequest).getParameter("edit-source");
		doReturn("/target").when(mockSlingRequest).getParameter("edit-target");
		...
		log.debug("Setting up the resource /etc/redirectMap.txt");
		...
		doReturn(IOUtils.toInputStream("/source1 /target1\n# Look a Comment\n/source2 /target2\n/source3 /target3"))
				.when(mockFileResource).adaptTo(InputStream.class);
	...
	public void testRemoveEntryServlet() throws ServletException, IOException {
		...
		log.info("REDIRECT MAP:\n" + value);
		assertFalse(value.contains("/source2 /target2"));
		assertTrue(value.contains("# Look a Comment"));
		log.info("Test successful!");
	}
}
