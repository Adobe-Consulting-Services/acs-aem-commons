...
public class ReplicatePageVersionServiceImplTest {
    ...
    @Test
    public final void testgetAppropriateVersion() throws Exception {
        ...
        when(contentNode.isNodeType(JcrConstants.MIX_VERSIONABLE)).thenReturn(
                true);
        when(contentNode.getPath()).thenReturn(
                path + "/" + NameConstants.NN_CONTENT);
    ...
    @Test
    public final void testgetResources() throws Exception {
        ...
        final List<Resource> resources = Arrays.asList(new Resource[] { res1,
                res2 });
    ...
    @Test
    public final void testfindAllVersions() throws Exception {
        final String vName = "version1";
    ...
    //
    @Test
    public final void testReplicateResource() throws Exception {
        ...
        verify(replicator, times(1)).replicate(eq(session),
                eq(ReplicationActionType.ACTIVATE), eq(path),
                any(ReplicationOptions.class));
        verifyNoMoreInteractions(replicator);
    }

    @Test
    public final void testGetAgent() throws Exception {
        ...
        final Collection<Agent> agentValues = Arrays.asList(new Agent[] {
                agent1, agent2 });
    ...
    @Test
    public final void testgetAgents() throws Exception {
        final Agent agent1 = mock(Agent.class);
        final Agent agent2 = mock(Agent.class);
        ...
        when(agentConfig1.getTransportURI()).thenReturn(
        ...
        when(agentConfig1.getProperties()).thenReturn(
                new ValueMapDecorator(tmp));
        Map<String, Object> tmp1 = new HashMap<String, Object>();
        tmp.put("reversereplication", "true");
        when(agentConfig2.getProperties()).thenReturn(
                new ValueMapDecorator(tmp1));
