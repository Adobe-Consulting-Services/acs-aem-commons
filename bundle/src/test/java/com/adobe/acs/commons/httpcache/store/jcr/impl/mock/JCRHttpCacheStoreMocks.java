...
public class JCRHttpCacheStoreMocks {
    ...
    private void mockStore() throws Exception {
        Whitebox.setInternalState(store, "cacheRootPath", JCRHttpCacheStoreImpl.Config.DEFAULT_ROOTPATH);
        Whitebox.setInternalState(store, "bucketTreeDepth", JCRHttpCacheStoreImpl.Config.DEFAULT_BUCKETDEPTH);
        Whitebox.setInternalState(store, "deltaSaveThreshold", JCRHttpCacheStoreImpl.Config.DEFAULT_SAVEDELTA);
        Whitebox.setInternalState(store, "expireTimeInMilliSeconds", JCRHttpCacheStoreImpl.Config.DEFAULT_EXPIRETIMEINMILISECONDS);
